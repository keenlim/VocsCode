...
describe.runIf(enabled)('electron e2e: Pi tool compatibility', () => {
  it('loads app resources, denies/executes aliases exactly once, and resumes after a real restart', async () => {
    ...
    try {
      ...
      for (const name of ['vocs-code-tools.ts', 'tool-arguments.ts', 'vocs-code-approvals.ts', 'vocs-code-subagents.ts', 'subagent-gate.ts', 'subagent-agents.ts', 'subagent-runs.ts']) {
